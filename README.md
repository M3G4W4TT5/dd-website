...
The personal site uses **Astro, React, TypeScript and CSS**. The studio site uses **Next.js, React and TypeScript**, with a local **pretix Community** availability preview backed by PostgreSQL and Redis. Hourly studio booking and payment are not yet live. The studio events calendar reads pretix events and dates, while event checkout remains gated until the payment and one-room inventory setup is verified. See [events integration](apps/booking/EVENTS.md) and [Stripe test setup](docs/STRIPE_TEST_SETUP.md).

## Contact forms

Both forms send through the booking app's `/api/contact` route, which must run on the VPS. The personal site can remain a static Cloudflare deployment; its browser code posts to `PUBLIC_BOOKING_URL/api/contact`. The booking app sends each message from and to the matching Purelymail mailbox, with the visitor in `Reply-To`. It then acknowledges the visitor from the same mailbox: personal inquiries use a DD. signature, while booking inquiries use a TTD Studio mark. Both acknowledgements include a plain text version.
...
The route uses Purelymail SMTP at `smtp.purelymail.com:465` with TLS. Cross-origin requests are allowed only from the configured personal origin; each origin can submit only for its own mailbox. The form includes a hidden spam trap and validates fields on the server. Each acknowledgement has a separate process-local cap of one reply per address per hour and 30 replies per hour in total; the inquiry itself is still delivered. Keep `apps/booking/public/branding/ttd-email-mark.png` alongside the booking app when deploying, because the TTD acknowledgement embeds it as an inline image. Production mail delivery cannot be verified until the server has the mailbox credentials.
