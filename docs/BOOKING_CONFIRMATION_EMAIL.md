...
Prepared content for paid bookings, booking changes, and cancellations. These are drafts for the local/hosted test shop; SMTP and live delivery are not enabled by this file. Configure the rental event's Danish and English **Paid order** content in pretix under **Event → Settings → E-mail → E-mail content**. Keep the placed-order/payment-pending message distinct from the paid confirmation.
...
4. Configure and test change, cancellation, and refund-status notices when the order/refund workflow is connected. A cancelled order and a completed refund are different states; do not say money was returned merely because cancellation succeeded. Verify the exact From address on all three message types.
